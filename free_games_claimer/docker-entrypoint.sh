...
# ── Home Assistant Options Parsing ──
if [ -f "/data/options.json" ]; then
    echo "Parsing Home Assistant options..."
    while IFS= read -r line; do
        if [ ! -z "$line" ]; then
            # Do not print passwords to log for security
    ...
    done < <(jq -r 'to_entries[] | select(.value != null and .value != "") | "\(.key|ascii_upcase)=\(.value)"' /data/options.json)
fi

# ── Persistent Data Storage Mapping ──
if [ -d "/config" ]; then
    echo "Symlinking persistent configuration folder (/config) to /fgc/data..."
    mkdir -p /config/browser /config/screenshots
    rm -rf /fgc/data
    ln -s /config /fgc/data
fi
